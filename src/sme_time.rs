...
pub fn sme_time_convert_to_timezone_impl(time_ob: u32, tz_str: &str) -> Option<String> {
    match tz_str.parse::<Tz>() {
        Ok(tz) => {
            ...
            Some(format_tz_result(
                &dt.format(_TIMEFMTSHORT).to_string(),
        ...
        Err(_) => {
            if tz_str.len() >= 4 {
                ...
                let offset: i32 = match prefix.as_str() {
                    // seconds
...
mod sme_time_tests {
    ...
    fn test_sme_time_as_string() {
        assert_eq!(
            sme_time_as_string_impl(1608594507_u32),
            "2020-12-21 23:48:27"
        );
    }
    ...
    fn test_sme_time_convert_to_timezone_tz_succeed() {
        assert_eq!(
            sme_time_convert_to_timezone_impl(1608594507_u32, "America/New_York"),
            Some("Mo 18:48,-18000".to_string())
        )
    }
    ...
    fn test_sme_time_convert_to_timezone_offs_succeed() {
        assert_eq!(
            sme_time_convert_to_timezone_impl(1608594507_u32, "utc-5"),
            Some("Mo 18:48,-18000".to_string())
        )
    }
}
